...
なぜこれが必要なのかと言えば、LUFはAWSのS3上や, Github上のコンテンツを Lambda関数URLから、それらリソースを呼び出して実行しますが、この環境で `Webアプリを開発` しようとすると、動作確認（テスト）作業において、非常に `面倒` な壁に突き当たります.
...
内容的には１つの工程が無いだけのように見えますが、一方で `コンテンツを所定のS3にUpdate` や `githubのコミット＋プルリク反映` これが「編集・確認作業」をする工程で、これらの頻度が増えるほど `大変で面倒` な作業となるわけで、この作業を開発を行う上で非常に「面倒」であるといえます.

そして githubに至っては、この「確認・編集作業」の度に githubにコミットしてpushを「毎回行う」事になり、現実的な開発環境では無いでしょう.
