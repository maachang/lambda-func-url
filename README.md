...
実装すると、関数URLにアクセスすると、以下のような内容がログ出力される(CloudWatchへ).
...
このような形で event が渡され `"event.rawPath": "/"` のアクセスパス＋ファイル名が取れるので、たとえば定義された関数URLのURLに対して、当該設定されたLambdaの `関数URL` の後に指定されたURLが、ここから取得できる.
...
また懸念点としては、Lambdaの関数URLの実験では、デプロイ後起動までに5～6秒ぐらいかかるが、それ以降は100ミリ秒単位ぐらいで返却されるので、普通に使えそう.
...
LFUのAWS Lambda側の実装は非常にシンプル具体的には `lmdLib/index.hsで、作成時のAWS Lambda関数定義に対して以下のように実装する事で対応できる.
